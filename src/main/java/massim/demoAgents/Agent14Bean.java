...
import java.util.*;
public class Agent14Bean  extends AbstractAgentBean{
    private String usname="12";
...
log.println("yek shodam" +" "+action+" in step "+step);
    ...
    public Agent14Bean(String host, String username, String pass, int port) {
    ...
    public void processRequestAction(Element perception, Element target, long currenttime, long deadline) throws IOException {
        ...
        int cnt=0;int cnt2=0;boolean f=false;

        action=randomAction();
        log.println("action agent"+username +" "+action+" in step "+step);
        log.flush();
        fw.flush();
